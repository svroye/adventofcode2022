...
def update_head(head_x, head_y, command):
    if command == 'R':
...
def update_knot(knot_to_follow, knot):
    x_1, y_1 = knot_to_follow
    x_2, y_2 = knot
    dx = x_1 - x_2
    dy = y_1 - y_2
    if dx == 0:
...
for command in data:
    dir_steps = command.split()
    direction = dir_steps[0]
    steps = int(dir_steps[1])

    for i in range(steps):
        head_x, head_y = update_head(head_x, head_y, direction)
        tail_x, tail_y = update_knot((head_x, head_y), (tail_x, tail_y))
...
# All knots start at the same position
knots = []
...
for command in data:
    ...
    for i in range(steps):
        ...
        knots[0] = update_head(head_x, head_y, direction)

        for j in range(1, len(knots)):
            knot_to_follow = knots[j - 1]
            current_knot = knots[j]
            knots[j] = update_knot(knot_to_follow, current_knot)
