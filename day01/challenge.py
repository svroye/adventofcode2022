...
for entry in data:
    if not entry:
        ...
        inventory[index] += int(entry)

sorted_descending = sorted(inventory.items(), key=lambda x: x[1], reverse=True)
print(sorted_descending[0])
...
print(sum([x[1] for x in sorted_descending[:3]]))