...
The monkeys take turns inspecting and throwing items. On a single monkey's turn, it inspects and throws all the items
...
class Operation:
    ...
    def __init__(self, operator, amount=None):
        self.operator = operator
        self.amount = amount

    # for part 1: add // 3
    # for part 2: remove ir
...
class Monkey:
    ...
    def __init__(self, sequence_number, operation: Operation = None):
    ...
    def apply_test(self, value):
        return value % self.test == 0

    def set_throw_to_true(self, value):
        self.throw_to_true = int(value)

    def set_throw_to_false(self, value):
        self.throw_to_false = int(value)
...
def get_operation(line: str):
    ...
    value = line.split(operator)[-1].strip()
    if value.isnumeric():
        return Operation(operator, int(value))
    else:
        return Operation(operator)


def get_items(line):
    return [int(entry) for entry in line[line.index(':') + 1:].split(', ')]
...
for line in data:
    if 'Monkey' in line:
        monkeys.append(Monkey(line[-2]))
    elif 'Starting items' in line:
        monkey = monkeys[-1]
        monkey.add_items(get_items(line))
    elif 'Operation' in line:
        monkey = monkeys[-1]
        monkey.set_operation(get_operation(line))
    elif 'Test' in line:
        monkey = monkeys[-1]
        monkey.add_test(Test(int(line.split(' ')[-1])))
    elif 'true' in line:
        monkey = monkeys[-1]
        throw_to = line.split(' ')[-1]
        monkey.set_throw_to_true(throw_to)
    elif 'false' in line:
        monkey = monkeys[-1]
        throw_to = line.split(' ')[-1]
        monkey.set_throw_to_false(throw_to)

count = 0

common_devisor = reduce((lambda x, y: x * y), [monkey.test.value for monkey in monkeys])
# common divisor of all tests

for monkey in monkeys:
    print(monkey.sequence_number, monkey.items, monkey.inspected_items)

while count < 10:
    for monkey in monkeys:
        while monkey.items:
            item = monkey.inspect_item()
            new_value = monkey.operation.apply(item)
            print('common_devisor', common_devisor)
            new_value = new_value % common_devisor
            if monkey.test.apply(new_value):
                monkeys[monkey.throw_to_true].add_items([new_value])
            else:
                monkeys[monkey.throw_to_false].add_items([new_value])
