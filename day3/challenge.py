...
with open('./input_1.txt', 'r') as f:
    ...
    data_compartments = []  # for first part
    for line in f.readlines():
        line = line.strip('\n')
        half_length = int(len(line) / 2)
        data.append(line)
        data_compartments.append([line[:half_length], line[half_length:]])

letter_score = {}
for s in string.ascii_uppercase:
    letter_score[s] = ord(s) - 38

for s in string.ascii_lowercase:
    letter_score[s] = ord(s) - 96

duplicate_items = []
for item in data_compartments:
    first_part = item[0]
    second_part = item[1]
    for el in first_part:
        if el in second_part:
            duplicate_items.append(letter_score[el])
            break

print(sum(duplicate_items))
...
for i in range(0, len(data), 3):
    first_part = data[i]
    second_part = data[i + 1]
    third_part = data[i + 2]
    for el in first_part:
        if el in second_part and el in third_part:
            group_badge_scores.append(letter_score[el])
            break

print(sum(group_badge_scores))
