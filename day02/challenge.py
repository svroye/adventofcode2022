...
score_dict = {0: 1, 1: 2, 2: 3}

score = 0
for game in games:
    opponent = options[game[0]]
    you = options[game[1]]
    if opponent == you:  # draw
        score += 3
    elif (opponent + 1) % 3 == you:  # you win
        score += 6
...
for game in games:
    opponent = options[game[0]]
    your_strategy = game[1]
    if your_strategy == 'X':  # LOSE
        you = (opponent + 2) % 3
    elif your_strategy == 'Y':  # DRAW
        you = opponent
    else:  # WIN
        you = (opponent + 1) % 3
    if opponent == you:  # draw
