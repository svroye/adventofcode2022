...
the middle of the second addx -1, so the value of register X is the starting value, 1, plus all the other addx
...
for entry in data:
    ...
    if len(entries) == 1:
        # noop
        cycle += 1
        cycle_dict[cycle] = cycle_dict[cycle - 1]
    else:
        # addx V
        value = int(entries[1])
        cycle_dict[cycle + 1] = cycle_dict[cycle]
        cycle_dict[cycle + 2] = cycle_dict[cycle] + value
        cycle += 2

cycles = [(i, cycle_dict[i]) for i in range(20, len(cycle_dict), 40)]

signal_strength = 0
for entry in cycles:
    signal_strength += entry[0] * entry[1]
...
the sprite is visible the instant each pixel is drawn. If the sprite is positioned such that one of its three pixels is 
the pixel currently being drawn, the screen produces a lit pixel (#); otherwise, the screen leaves the pixel dark (.).
...
for i in range(screen_height):
    for j in range(screen_width):
        x_middle = cycle_dict[j + 40 * i + 1]

        #print(f"i = {i}, j = {j}, x_middle = {x_middle}, new x_middle = {cycle_dict[j + 40 * i + 1]}")
        if abs(j - x_middle) <= 1:
            screen[i][j] = '#'
