...
for line in data:
    ...
    for i in range(quantity):
        entry = stack[from_stack - 1][-1]
        stack[to_stack - 1].append(entry)
        del stack[from_stack - 1][-1]
...
for line in data:
    ...
    from_stack = int(split_line[3])
    to_stack = int(split_line[5])
    from_stack_entries = stack[from_stack - 1]
    entries = from_stack_entries[len(from_stack_entries) - quantity:]
    stack[to_stack - 1].extend(entries)
    del from_stack_entries[len(from_stack_entries) - quantity:]
